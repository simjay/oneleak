"""Real end-to-end tests: invoke `oneleaks` as an actual subprocess (real
...
Uses `sys.executable -m oneleaks.cli` rather than the installed `oneleaks`
...
ONELEAKS = [sys.executable, "-m", "oneleaks.cli"]


def run_oneleaks(args, *, cwd=None, input_text=None, env=None):
...
class TestSmoke:
    def test_version(self):
        result = run_oneleaks(["--version"])
        assert result.returncode == 0
        assert "oneleaks" in result.stdout

    def test_help(self):
        result = run_oneleaks(["--help"])
        assert result.returncode == 0
        assert "scan" in result.stdout
        assert "sanitize" in result.stdout

    def test_scan_help(self):
        result = run_oneleaks(["scan", "--help"])
        assert result.returncode == 0
...
class TestScanViaSubprocess:
    def test_clean_stdin_exit_0(self):
        result = run_oneleaks(["scan", "-"], input_text="hello world\n")
        assert result.returncode == 0
    ...
    def test_scan_a_real_file(self, tmp_path: Path):
        ...
        result = run_oneleaks(["scan", str(f)])
        assert result.returncode == 1
        assert "openai-api-key" in result.stdout
    ...
    def test_scan_a_real_directory(self, tmp_path: Path):
        ...
        result = run_oneleaks(["scan", str(tmp_path)])
        assert result.returncode == 1
    ...
    def test_malformed_config_exits_2_with_no_traceback(self, tmp_path: Path):
        ...
        result = run_oneleaks(["scan", str(target), "--config", str(bad_config)])
        assert result.returncode == 2
        assert "Traceback" not in result.stderr
...
class TestSanitizeDesanitizeRoundTrip:
    def test_full_round_trip_through_real_files_and_pipes(self, tmp_path: Path):
        ...
        sanitized = run_oneleaks(["sanitize", str(src), "--map", str(map_path)])
...
class TestBaselineWorkflowViaSubprocess:
    def test_requires_stable_fingerprint_key(self, tmp_path: Path):
        env = {k: v for k, v in os.environ.items() if k != "ONELEAKS_FINGERPRINT_KEY"}
        result = run_oneleaks(
        ...
        assert result.returncode == 2
        assert "ONELEAKS_FINGERPRINT_KEY" in result.stderr

    def test_update_then_recheck_is_clean_then_new_secret_is_reported(self, tmp_path: Path):
        env = {**os.environ, "ONELEAKS_FINGERPRINT_KEY": "e2e-stable-key"}
        target = tmp_path / "app.py"
        target.write_text("OPENAI_API_KEY = 'sk-proj-" + "a" * 20 + "'\n")
        baseline = tmp_path / ".oneleaks-baseline.json"
        ...
        recheck = run_oneleaks(["scan", str(target), "--baseline", str(baseline)], env=env)
        assert recheck.returncode == 0

        target.write_text(target.read_text() + "\nANTHROPIC_API_KEY = 'sk-ant-" + "b" * 20 + "'\n")
        with_new_secret = run_oneleaks(["scan", str(target), "--baseline", str(baseline)], env=env)
...
class TestGitScanningViaSubprocess:
    ...
    def test_staged_scan(self, tmp_path: Path):
        ...
        _git(["add", "-A"], tmp_path)
        result = run_oneleaks(["scan", "--staged"], cwd=tmp_path)
        assert result.returncode == 0
    ...
    def test_changed_scan(self, tmp_path: Path):
        ...
        result = run_oneleaks(["scan", "--changed"], cwd=tmp_path)
        assert result.returncode == 1
    ...
    def test_history_scan_finds_removed_secret(self, tmp_path: Path):
        ...
        clean_scan = run_oneleaks(["scan", str(tmp_path)])
        assert clean_scan.returncode == 0  # gone from the working tree

        history_scan = run_oneleaks(["scan", "--history"], cwd=tmp_path)
...
class TestConfigDiscoveryFromRealCwd:
    def test_oneleaks_yaml_is_auto_discovered_and_custom_rule_fires(self, tmp_path: Path):
