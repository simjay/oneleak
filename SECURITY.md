...
oneleaks handles sensitive data (secrets and PII) by design, so security issues in it are taken seriously.
...
Instead, report it privately via [GitHub Security Advisories](https://github.com/simjay/oneleaks/security/advisories/new) for this repository. Include:
...
Given oneleaks's purpose, these are treated as security issues even if they'd be "just bugs" elsewhere:
...
- Any way for `.oneleaks.yaml` to trigger code execution or network access
...
Only the latest released version is supported with security fixes while oneleaks is pre-1.0.
