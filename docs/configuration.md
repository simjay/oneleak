...
```bash
oneleak scan . --baseline .oneleak-baseline.json --fail-on high
```
...
Baseline matching relies on `Finding.fingerprint`, which is an HMAC keyed off a random value generated fresh **per process** unless `ONELEAK_FINGERPRINT_KEY` is set. Without a stable key, fingerprints computed today would never match fingerprints computed tomorrow (or in CI, or on a teammate's machine), so the baseline would silently never match anything. `oneleak scan --baseline` refuses to run at all until `ONELEAK_FINGERPRINT_KEY` is set, rather than fail that way silently.
