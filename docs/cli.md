...
```bash
oneleak --version   # print the installed version and exit
oneleak --help      # top-level usage (each subcommand also takes --help)
```
...
oneleak scan . --baseline .oneleak-baseline.json --update-baseline   # accept today's findings
oneleak scan . --baseline .oneleak-baseline.json                     # only report new ones
```
...
| `--baseline PATH` | Only report findings not already recorded in this baseline file. Requires `ONELEAK_FINGERPRINT_KEY` to be set. See [Baselines](configuration.md#baselines). |
| `--update-baseline` | With `--baseline`: overwrite it with this run's findings instead of filtering against it. |

### Git history scanning

`oneleak scan .`, `--changed`, and `--staged` only see *current* content. A secret committed and later removed is still fully recoverable from git history unless it's been rewritten, and `--history` is what catches that:
